...
class RunnerConfig:
    ...
    def load_model(self, quantization_type):
        ...
        if quantization_type == "32-bit":
            model = LLAMA(
                "/data/.tmp/Loong/run_models/onnx/models/Qwen2.5-7B-Instruct-32bit-ONNX",
                512,
            )
        elif quantization_type == "16-bit":
            model = LLAMA(
                "/data/.tmp/Loong/run_models/onnx/models/Qwen2.5-7B-Instruct-16bit-ONNX",
                512,
            )
        elif quantization_type == "awq-4-bit":
            model = LLAMA(
                "/data/.tmp/Loong/run_models/onnx/models/Qwen2.5-7B-Instruct-AWQ-4bit-ONNX",
                512,
            )
        elif quantization_type == "gptq-4-bit":
            model = LLAMA(
                "/data/.tmp/Loong/run_models/onnx/models/Qwen2.5-7B-Instruct-GPTQ-4bit-ONNX",
                512,
            )
